...
[절대 형식 규칙 - 위반 금지]
- "1. 역할", "2. 목표", "3. 조건", "4. 출력 형식"은 반드시 한 줄로 작성할 것
- "2.\n목표"처럼 줄바꿈 절대 금지
- 번호와 항목명을 분리하지 말 것
- 각 항목은 "제목 1줄 + 다음 줄 본문" 구조 유지
...
2. 역할
- "1. 역할" 항목의 내용을 반드시 그대로 유지할 것
- 반드시 문장 시작을 "○○ 전문가로서," 형태로 시작할 것
- 역할 표현은 반드시 한 번만 사용
- "전문가로서" 단독 사용 절대 금지
- "전문가로서"가 이미 포함된 경우 중복 생성 금지
- 역할이 누락되거나 일반화되면 잘못된 결과로 간주할 것
...
[역할 처리 규칙]
- 기존 역할이 이미 존재하면 절대 중복 생성하지 말 것
- "전문가로서" 표현이 이미 있으면 추가하지 말 것
- 역할 표현은 반드시 한 번만 사용
...
[중복 제거 규칙 - 매우 중요]
- "전문가로서 전문가로서" 형태 절대 금지
- 역할 표현이 2번 이상 나오면 1개만 남기고 제거할 것
