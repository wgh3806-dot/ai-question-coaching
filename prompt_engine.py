...
def get_style_instruction(style):
    if style == "간결형":
        ...
        return """
                ...
                - 역할(Role), 목표(Goal), 조건(Instructions), 출력 형식(Format)의 핵심 내용을 모두 포함할 것
                - 1. 2. 3. 4. 번호 구조로 나열하지 말 것
                - 항목형 나열보다 연결된 한두 개 단락의 요청문 형태를 우선할 것
                - 공문, 안내문, 의뢰문처럼 쓰지 말 것
                - '귀하', '요청드립니다', '직원으로서', '~로서' 같은 표현은 사용하지 말 것
                - 문장형 프롬프트에도 반드시 역할 부여를 포함할 것
                - 역할은 문장 첫부분에 자연스럽게 녹여 쓸 것
                - 예: '정보통신 선진지 견학 장소 추천 전문가로서 ... 추천해줘' 같은 방식은 허용할 것
                - 단, '너는 ~ 전문가야' 같은 어색한 회화체는 사용하지 말 것
                - 사용자가 AI에게 직접 지시하는 실전 요청문처럼 '~해줘', '~추천해줘', '~정리해줘', '~작성해줘' 형태로 자연스럽게 끝낼 것
