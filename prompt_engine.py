...
def get_style_instruction(style):
    if style == "간결형":
        return """
                출력 형식:
                - 1.역할, 2.목표, 3.조건, 4.출력 형식 구조는 유지할 것
        ...
        return """
                ...
                - 1.역할, 2.목표, 3.조건, 4.출력 형식의 핵심 내용을 모두 포함할 것
                - 번호형 구조로 나열하지 말고, 한두 문단의 연결된 문장으로 자연스럽게 작성할 것
                - 프롬프트 자체는 읽기 쉽게 정리하되, 핵심 정보가 빠지지 않게 할 것
                - 반드시 문장 첫 부분에 구조형의 역할(Role)을 그대로 사용하여 "○○ 전문가로서" 형태로 포함할 것
                - 구조형에서 정의된 역할을 임의로 변경하거나 새로 생성하지 말 것
        ...
        return """
                출력 형식:
                - 1.역할, 2.목표, 3.조건, 4.출력 형식 형식 구조 유지
                - 각 항목은 짧고 간단하게 작성할 것
                - 조건은 3개 이내로 제한
                - 불필요한 설명 금지
                - 실무에서 바로 사용할 수 있게 작성
...
def generate_prompt(preview_text, style, max_tokens=500):
    ...
    system_prompt = f"""
        ...
        - 조건(Instructions)은 3~4개 이내로 핵심만 작성할 것
...
- 역할, 목표, 조건, 출력 형식을 빠뜨리지 말 것
- 결과는 항목 나열형보다 자연스러운 요청문 형태로 만들 것
- 절대 "~입니다", "~해주세요" 같은 설명형/존댓말 표현 사용 금지
- 반드시 사용자가 AI에게 직접 지시하는 요청문 형태로 작성할 것 (~해줘, ~작성해줘 형태)
